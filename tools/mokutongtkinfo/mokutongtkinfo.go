...
func main() {
	...
	alphabeticalOrder := flag.Bool("a", false, "alphabetical ascending order")
	githubCode := flag.Bool("g", false, "prepend spaces to each line and formatted as code")
	flag.Parse()
	fname := flag.Arg(0)
	if fname == "" {
		fmt.Println("Usage: mokutongtkinfo [-ag] file")
		return
	}
