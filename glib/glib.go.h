...
static inline GObject* to_GObject(void* o) { 
	return G_OBJECT(o); 
}

static inline gchar* to_gcharptr(const char* s) { 
	return (gchar*)s; 
}

static inline char* to_charptr(const gchar* s) { 
	return (char*)s; 
}

static inline char* to_charptr_from_gpointer(gpointer s) { 
	return (char*)s; 
}

static inline char* to_charptr_voidp(const void* s) { 
	return (char*)s; 
}
...
static inline gchar* next_string(gchar* s) { 
	return (gchar*)(s + strlen(s) + 1); 
}
...
static void g_value_init_int(GValue* gv) { 
	g_value_init(gv, G_TYPE_INT); 
}

static void g_value_init_string(GValue* gv) { 
...
static GValue* init_gvalue_uint(guint val) { 
	GValue* gv = g_new0(GValue, 1); 
	g_value_init(gv, G_TYPE_UINT); 
	g_value_set_uint(gv, val); 
	return gv; 
}

static GValue* init_gvalue_double(gdouble val) { 
	GValue* gv = g_new0(GValue, 1); 
	g_value_init(gv, G_TYPE_DOUBLE); 
	g_value_set_double(gv, val); 
	return gv; 
}

static GValue* init_gvalue_byte(guchar val) { 
	GValue* gv = g_new0(GValue, 1); 
	g_value_init(gv, G_TYPE_UCHAR); 
	g_value_set_uchar(gv, val); 
	return gv; 
}

static GValue* init_gvalue_bool(gboolean val) { 
	GValue* gv = g_new0(GValue, 1); 
...
static gboolean _glib_callback(void *data, ...) {
	...
	free(cbi->args); //not sure if this will create doublefree, sorry but it must be freed :'-(
