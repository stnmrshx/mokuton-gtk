...
//Dear pygtksourceview, again, thankyou for the constanta :))
const (
	...
	SOURCE_DRAW_SPACES_ALL      SourceDrawSpacesFlags = (SOURCE_DRAW_SPACES_SPACE | SOURCE_DRAW_SPACES_TAB | SOURCE_DRAW_SPACES_NEWLINE | SOURCE_DRAW_SPACES_NBSP | SOURCE_DRAW_SPACES_LEADING | SOURCE_DRAW_SPACES_TEXT | SOURCE_DRAW_SPACES_TRAILING)
)

func (v *SourceView) SetDrawSpaces(flags SourceDrawSpacesFlags) {
	C.gtk_source_view_set_draw_spaces(v.ToNativeSourceView(), C.GtkSourceDrawSpacesFlags(flags))
}
...
func (v *SourceView) SetTabWidth(width uint) {
	C.gtk_source_view_set_tab_width(v.ToNativeSourceView(), C.guint(width))
}
...
func (v *SourceView) SetSmartHomeEnd(flags SourceSmartHomeEndType) {
	C.gtk_source_view_set_smart_home_end(v.ToNativeSourceView(), C.GtkSourceSmartHomeEndType(flags))
}
...
// gtkSourceLanguage encapsulates syntax and highlighting styles for a particular language
...
// GtkSourceLanguageManager object which processes language description files and creates and stores GtkSourceLanguage objects
